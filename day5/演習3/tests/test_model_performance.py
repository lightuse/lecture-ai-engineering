...
def load_baseline_model(load_current_model):
    ...
    if not os.path.exists(BASELINE_MODEL_PATH):
        ...
        print(
            f"ベースラインモデルが存在しないため、現在のモデルをコピーしました: {BASELINE_MODEL_PATH}"
        )
...
def test_model_performance_metrics(load_dataset, load_current_model):
    ...
    ), f"精度が閾値を下回っています: {metrics['accuracy']:.4f} < {ACCURACY_THRESHOLD}"
    assert (
        metrics["precision"] >= PRECISION_THRESHOLD
    ), f"適合率が閾値を下回っています: {metrics['precision']:.4f} < {PRECISION_THRESHOLD}"
    assert (
        metrics["recall"] >= RECALL_THRESHOLD
    ), f"再現率が閾値を下回っています: {metrics['recall']:.4f} < {RECALL_THRESHOLD}"
    assert (
        metrics["f1"] >= F1_THRESHOLD
    ), f"F1スコアが閾値を下回っています: {metrics['f1']:.4f} < {F1_THRESHOLD}"
    assert (
        metrics["inference_time"] <= INFERENCE_TIME_THRESHOLD
    ), f"推論時間が閾値を超えています: {metrics['inference_time']:.4f} > {INFERENCE_TIME_THRESHOLD}"
...
def test_compare_with_baseline(load_dataset, load_current_model, load_baseline_model):
    ...
    for metric in ["accuracy", "precision", "recall", "f1", "roc_auc"]:
        if (
            current_metrics[metric] < 0.95 * baseline_metrics[metric]
        ):  # 5%以上の性能劣化を検出
    ...
    # 推論時間は小さい方が良い
    if (
        current_metrics["inference_time"] > 1.2 * baseline_metrics["inference_time"]
    ):  # 20%以上の性能劣化を検出
        performance_degradation = True
        degradation_messages.append(
